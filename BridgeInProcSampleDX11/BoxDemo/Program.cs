...
namespace BoxDemo {
    ...
    void UninitBridge()
    {
       BridgeInProc.Controller.UnregisterTextureDX(_bridge_window, _quiltRenderTarget.ComPointer);
        ...
        if (window_showing)
        {
            BridgeInProc.Controller.ShowWindow(_bridge_window, false);
        }
    ...
    {
        // mlc: init bridge
        if (!BridgeInProc.Controller.Initialize(@"BridgeInProcSampleDX11"))
        {
            Environment.Exit(-1);
        }

        // mlc: instance the window
        bool window_status = BridgeInProc.Controller.InstanceWindowDX(Device.ComPointer, ref _bridge_window);
    ...
    {    
        ...
        public override void DrawScene()
        {
            ...
            {
                BridgeInProc.Controller.ShowWindow(_bridge_window, window_showing);
            ...
            if (inited)
            {
                ...
                if (save)
                {
                    BridgeInProc.Controller.SaveTextureToFileDX(_bridge_window, @"C:\temp\quilt.png", _quiltRenderTarget.ComPointer);
                }
            }
