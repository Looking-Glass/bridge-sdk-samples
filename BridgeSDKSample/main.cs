...
namespace RotatingCube
{
    ...
    public class MainWindow : GameWindow
    {
        ...
        protected override void OnLoad()
        {
            ...
            // mlc: init bridge
            if (!BridgeSDK.Controller.Initialize(@"BridgeSDKSample"))
            {
                Environment.Exit(-1);
            }

            // mlc: instance the window
            bool window_status = BridgeSDK.Controller.InstanceWindowGL(ref _bridge_window);

            if (!window_status)
            {
                throw new Exception("Tried to create a Bridge Window without a client GL context active!");
            }
        ...
        protected override void OnUpdateFrame(FrameEventArgs args)
        {
            ...
            for (uint y = 0; y < _bridge_quilt_vy; y++)
            {
                ...
                {
                    GL.Viewport((int)(x * _bridge_quilt_view_width), (int)(y * _bridge_quilt_view_height), (int)_bridge_quilt_view_width, (int)_bridge_quilt_view_height);
            ...
            // mlc: draw desktop head preview
            GL.BindFramebuffer(FramebufferTarget.Framebuffer, 0);

            // mlc: debug -- see what the quilt looks like
            //BridgeSDK.Controller.SaveTextureToFileGL(_bridge_window, @"C:\\temp\\quilt_gl.png", (ulong)_bridge_render_texture, BridgeSDK.PixelFormats.RGBA, _bridge_render_texture_width, _bridge_render_texture_height);
            ...
            BridgeSDK.Controller.DrawInteropQuiltTextureGL(_bridge_window, (ulong)_bridge_render_texture, BridgeSDK.PixelFormats.RGBA, _bridge_render_texture_width, _bridge_render_texture_height, _bridge_quilt_vx, _bridge_quilt_vy, 1.0f, 1.0f);
