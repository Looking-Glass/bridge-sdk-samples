...
namespace BoxDemo {
    ...
    {
        // mlc: init bridge
        if (!BridgeSDK.Controller.Initialize(@"BridgeSDKSampleDX11Interactive"))
        {
            Environment.Exit(-1);
        }

        // mlc: instance the window
        bool window_status = BridgeSDK.Controller.InstanceOffscreenWindowDX(Device.ComPointer, ref _bridge_window);
